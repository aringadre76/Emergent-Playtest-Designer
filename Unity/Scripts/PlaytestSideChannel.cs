...
public class PlaytestSideChannel : SideChannel
{
    ...
    void Start()
    {
        recorder = GameObject.FindObjectOfType<PlaytestRecorder>();
    ...
    {
        if (recorder != null && !string.IsNullOrEmpty(parameters.outputPath))
        {
            recorder.StartRecording(parameters.outputPath);
        }
    }
