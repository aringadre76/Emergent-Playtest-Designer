...
public class PlaytestAgent : Agent
{
    ...
    private float CalculateReward()
    {
        ...
        if (playtestManager != null)
        {
            reward += playtestManager.CalculateGameSpecificReward(transform.position, rb.linearVelocity);
        }
    ...
    private void CheckForExploits()
    {
        if (exploitDetector != null)
        {
            exploitDetector.CheckExploits(transform.position, rb.linearVelocity, lastActions);
        }
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject.CompareTag("Wall") && rb.linearVelocity.magnitude > 10f)
        {
